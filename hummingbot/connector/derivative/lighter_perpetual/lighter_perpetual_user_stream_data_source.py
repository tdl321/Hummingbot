...
class LighterPerpetualUserStreamDataSource(UserStreamTrackerDataSource):
    """
    User stream data source for Lighter Perpetual.
    ...
    async def listen_for_user_stream(self, output: asyncio.Queue):
        ...
        Connects to Lighter WebSocket and processes user events.
        """
        while True:
            try:
                ...
                async for msg in ws.iter_messages():
                    ...
                    # Lighter WebSocket message format (to be verified):
    ...
    async def _subscribe_to_channels(self, ws: WSAssistant):
        ...
        try:
            ...
            self.logger().info("Subscribed to Lighter user stream")
        except Exception:
            self.logger().error("Failed to subscribe to Lighter user stream channels")
            raise
